...
class FileUploader
{
     ...
     * @param string|null $subdirectory Sous-dossier (ex: challenge, avatar...)
     * @return string le nom du fichié uploader
     * @throws FileException
     */
    public function upload(UploadedFile $file, ?string $subdirectory = null): string
    {
        $allowedMimeTypes = ['image/avif', 'image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml'];

        //Verifie que le file traité est au bon format 
        if (!in_array($file->getMimeType(), $allowedMimeTypes)) {
            throw new FileException(
                sprintf(
                    "Type de fichier non autorisé. Type accepté: %s",
        ...
        if ($file->getSize() > $maxSize) {
            throw new FileException("Fichier trop volumineux. Taille maximale: 5MB");
        }
        ...
        if (!is_writable($uploadPath)) {
            throw new FileException("Le dossier d'upload n'est pas accessible en ecriture: " . $uploadPath);
        }

        //On déplace le fichier physqiue dans sa cible
        try {
        ...
        //retourner le chemin avec le prefixe /uploads/ pour l'accès web
        if($subdirectory){
